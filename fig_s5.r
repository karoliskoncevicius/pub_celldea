...
res <- readRDS("data/ewasoverlaps_5categories.rds")
...
pdf("fig_s5.pdf", width = 12/2.54, height = nlevels(res$pmid)/4.5/2.54, pointsize = 8)
...
points(log2(res$odds_age), res$pmid, pch = ifelse(res$pval_age <= 0.05, 19, 1), col = "orange")
points(log2(res$odds_both), res$pmid, pch = ifelse(res$pval_age <= 0.05, 19, 1), col = "purple")
