...
dfage <- readRDS("diffage_cgs_5categories.rds")
...
is_both <- dfage$category == "age-ct"
is_age  <- dfage$category == "age-only"
is_stat <- dfage$category == "ct-only"
...
for(s in unique(ewas$studyid)) {
  ...
  # aging
  fish <- fisher.test(mytable(rownames(dfage) %in% ewas$cpg[ewas$studyid == s], is_age))
  res$odds_age[res$study == s] <- fish$estimate
  res$pval_age[res$study == s] <- fish$p.value
  res$perc_age[res$study == s] <- mean(is_age[rownames(dfage) %in% ewas$cpg[ewas$studyid == s]]) * 100
  # both
  fish <- fisher.test(mytable(rownames(dfage) %in% ewas$cpg[ewas$studyid == s], is_both))
  res$odds_both[res$study == s] <- fish$estimate
  res$pval_both[res$study == s] <- fish$p.value
  res$perc_both[res$study == s] <- mean(is_both[rownames(dfage) %in% ewas$cpg[ewas$studyid == s]]) * 100
  # rest
...
saveRDS(res, "ewasoverlaps_5categories.rds")
