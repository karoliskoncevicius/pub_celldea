...
masks <- read.delim("input/EPICv2.hg38.mask.tsv")
...
# masked probes
X <- X[!(X@id %in% masks$Probe_ID[masks$M_general]), ]
...
# bad snp info
S <- S[,colnames(X)]
snpcor <- cor(S); diag(snpcor) <- NA
X <- X[,is.na(X$donor) | X$donor %in#% 1 | X$donor == X$donor[apply(snpcor, 1, which.max)]]
...
# poor purity
C   <- C[colnames(X),]
C   <- cbind(C, tcell = C[,"tcd4n"] + C[,"tcd4m"] + C[,"tcd8n"] + C[,"tcd8m"] + C[,"treg"])
pur <- C[cbind(colnames(X), X$celltype)]
X   <- X[,ave(pur, X$celltype, FUN = scale) > -3]

# poor inter array correlation
