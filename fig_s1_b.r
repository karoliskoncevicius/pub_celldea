...
agedir <- ifelse(sigs[,"pvalue_mono"], "mono", "")
agedir <- paste0(agedir, ifelse(sigs[,"pvalue_tcd4n"], " tcd4n", ""))
agedir <- paste0(agedir, ifelse(sigs[,"pvalue_tcd4m"], " tcd4m", ""))
...
pdf("fig_s1_b.pdf", width = 14/2.54, height = 6/2.54, pointsize = 8)
...
cols <- c("white", cell2col("mono"))[grepl("mono", names(tab))+1]
cols <- c(cols, c("white", cell2col("tcd4n"))[grepl("tcd4n", names(tab))+1])
cols <- c(cols, c("white", cell2col("tcd4m"))[grepl("tcd4m", names(tab))+1])
symbols(rep(1:8, 3), rep(-c(3000,6000,9000), each = 8), circles = rep(0.19, 3*8), bg = cols, add = TRUE, inches = FALSE)
text(0, -c(3000,6000,9000), c("Mono", "TCD4n", "TCD4m"), pos = 2, xpd = NA)
